...
clean_labor = function(file){
  ...
  labor_raw_dt = read_xlsx(
    path = here(paste0("data/bls-labforce/raw/",file))
  ) |> data.table()
  ...
  labor_raw_dt[,':='(year = file_yr, x = NULL, unemploy_ment_rate_percent = NULL)]
...
# Cleaning all files 
labor_dt = map_dfr(
  list.files(here("data/bls-labforce/raw")),
  clean_labor
)

labor_dt[,year := as.integer(year)]


# Saving the results 
write.fst(
  labor_dt, 
  path = here("data/bls-labforce/labor-dt.fst")
)
